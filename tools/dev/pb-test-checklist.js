...
(() => {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      items: [
        ["dr-backstabber", "Back Stabber shows a +1d2 damage toggle", "Attack with any weapon; tick it → +d2 in the damage roll + card note."],
        ["dr-focusedaim", "Focused Aim (taken ×2) shows +1d4 on ranged only", "Melee weapon → no +d4; ranged → +d4 toggle. Taken once → no +d4 at all."],
        ["dr-of-duel", "Ostentatious Fencer shows +1 dueling on rapier/cutlass only", "Dagger → toggle absent."],
        ["dr-focusedaim-gate", "Focused Aim −4 DR shows on ranged weapons only", "Was any-weapon before; now ranged-gated."],
      ],
    },
    {
      title: "2 · Blood Frenzy (countable)",
      items: [["bf-count", "Blood Frenzy shows a number field, applies +2×N", "Enter 3 → +6 damage + card note. Enter 0 → nothing."]],
    },
    {
      title: "3 · Burglar DR button",
      items: [
        ["burglar-4", "Burglar gives a −4 ability-test DR button", "Roll any ability → dialog checkbox applies −4."],
        ["burglar-6", "Taken ×2 shows −6 (not −8)", "Uneven progression, base+increment."],
      ],
    },
    {
      title: "4 · Crit range & immunity",
      items: [
        ["cc-crit", "Calculating Cutthroat crits on natural 19 (attack & defend)", "Roll a 19 → critical."],
        ["surv-immune", "Survivalist blocks Poisoned/Diseased/Infected", "Try to apply → 'immune' notice. Blind etc. still applies."],
      ],
    },
    {
      title: "5 · Black Powder Poet",
      items: [["bpp-bombs", "Dragging BPP onto a fresh character grants d4 bombs, once", "Drawn from the Grenades & Bombs table."]],
    },
    {
      title: "6 · Inspiring Leader (header chip)",
      items: [
        ["il-roll", "Use button rolls d4 to chat + a header chip appears (±N)", "Features tab → Use."],
        ["il-dismiss", "Chip tooltip shows roll time; ✕ clears it", "Header stays clean when no active roll."],
      ],
    },
    {
      title: "7 · Grog Brewer (full loop)",
      items: [
        ["grog-rest", "Long Rest auto-brews d4 grog (in the rest card)", "Grog stock increases."],
        ["grog-button", "Combat-tab Brew button; confirms if already brewed today", "No → nothing; Yes → brews again."],
        ["grog-soak", "'Soak blade in grog' toggle on melee attack; spends a serving", "Only shows with grog in stock + melee weapon."],
        ["grog-onhit", "On a hit: '☠ Toughness DR14 or −d6' + Roll poison damage button", "Poison d6 ignores armor."],
        ["grog-miss", "Miss while soaked: serving spent, no poison line", "Correct behaviour."],
        ["grog-rank2", "Taken ×2 draws a card: Red → d8 die, Black → DR 16", "Drop to ×1 → upgrade clears."],
      ],
    },
    {
      ...
      items: [
        ...
        ["ui-defbonus", "Defend dialog shows OF +2 as a bold line", ""],
        ["ui-armordr", "Header shows Armor DR (incl. Sea Turtle/Thick Skinned) + Defense", ""],
    ...
    },
    {
      ...
      items: [
        ["reg-of", "Ostentatious Fencer = a single effect row; +2 only with rapier/cutlass", "No duplicate."],
        ["reg-passives", "Shakespeare (+1 dmg) and Dazzling (+2 def) actually apply", ""],
    ...
    const items = section.items
      .map(([id, text, how]) => {
        ...
        return `<label class="pb-chk-item">
          ...
          <span class="pb-chk-body"><span class="pb-chk-text">${esc(text)}</span>${howHtml}</span>
        </label>`;
      })
      .join("");
